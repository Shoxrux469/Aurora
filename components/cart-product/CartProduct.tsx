...
const CartProduct = ({ item, onDelete, onIncrease, onDecrease, onQuantityChange }: props) => {
    ...
    <div className="flex items-start space-x-4 ">
      ...
      <div className="flex-1 flex items-start gap-1">
        <div className='max-w-[340px] w-full'>
          <h3 className="text-lg text-balance leading-5">
            {item.title}
          </h3>
          <p className="mt-1 text-sm text-muted-foreground">{item.attributes.color}</p>
      ...
      <div className="flex flex-col items-end space-y-2">
        <div className="text-lg font-medium">${item.price * item.cartQuantity}.00</div>
        <div className="text-sm text-muted-foreground line-through">${item.price + 9}.99</div>
        <Button
          variant="ghost"
          size="icon"
